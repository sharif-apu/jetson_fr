...
from utils.bfm import BFMModel
...
class TDDFA(object):
    ...
    def __init__(self, **kvs):
        ...
        ##print("path", make_abs_path('utils/configs/bfm_noneck_v3.pkl'))
        # load BFM
        self.bfm = BFMModel(
            bfm_fp='utils/configs/bfm_noneck_v3.pkl',#kvs.get('bfm_fp',  make_abs_path('configs/bfm_noneck_v3.pkl')),,
        ...
        # )
        #model = load_model(model, kvs.get('checkpoint_fp')).cuda()
        #x = torch.ones((1, 3, 120, 120)).cuda()
        # nFrame = 1000
        # start = time.time()
        ...
        # print("FPS (Without TRT):", nFrame/(time.time()-start))

        opt_model = "utils/weights/3ddfa_trt.pth"
